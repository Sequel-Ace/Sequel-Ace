...
- **Shared connection / session state.** Tools run on the same live connection as the targeted Sequel Ace tab, so per-connection session state is shared in both directions. In read-only mode the statements that change it (`USE`, `SET`, `BEGIN`/`START TRANSACTION`, `LOCK TABLES`, `CREATE TEMPORARY TABLE`) are rejected, but with read-only mode off a tool query could change the tab's current database, session variables, open transaction, or locks - and even a read clobbers per-connection results like `ROW_COUNT()`, `FOUND_ROWS()` and warnings. Keep read-only mode on, and don't rely on connection-scoped state (user variables, `LAST_INSERT_ID()`, temporary tables, named locks) being stable across the tab and the agent.
