...
3. **SSH tunnel IPC: NSConnection -> socket transport** — 🟡 in execution.
   ...
   (5b, #2623, which also turns the assistant into `main.swift` and takes the
   NSConnection warnings to zero) is a draft that waits for a release of
   soak. Highest blast radius in the codebase — SSH connections break if it
   is wrong — so DO stays as the rollback (`SPSSHTunnelUseSocketTransport
   -bool NO`) until then.
