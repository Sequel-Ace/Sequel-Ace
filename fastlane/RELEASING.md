...
For every release created before the cutoff, artifact publication is a bounded
manual compatibility handoff:
...
2. The workflow exits successfully with the exact wake tag still armed. Its
   ...
   another Mac runner.
3. An agent using Jason's signed-in GitHub release editor uploads the exact
   manifest-named ZIP or ZIPs. This is the last-resort UI step; neither the
   publisher PAT nor a browser session is stored in Actions.
4. The local release skill performs a supported REST read-back and requires the
   exact release/tag, Jason author and stable ID, JSON-null asset label, Jason
   uploader and legacy enum-constrained identity fields, `application/zip`,
...
this contract. At and after the fixed cutoff, new primary-repository assets use
the supported REST upload endpoint with a fresh release App token and the
manual handoff disappears.
...
private manifest and, during the compatibility epoch, require the exact
legacy-decodable author, uploader, label, type, and state metadata. The release
...
- Before the August 2027 cutoff, the hosted Mac archives verified ZIPs before
  public mutation and then stops if the exact GitHub assets are missing. The
  local skill performs the last-resort Jason UI upload and a strict API
  ...
  reverifies the artifacts. At and after the cutoff, the release App uses the
  supported asset API automatically.
